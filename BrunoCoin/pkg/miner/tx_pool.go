...
func CalcPri(t *tx.Transaction) uint32 {
	if (t == nil) {
		fmt.Printf("ERROR {CalcPri}: received nil as input.\n")
		return 0
	}
	var factor uint32 = 100
	fee := t.SumInputs() - t.SumOutputs()
	if t.Sz() == 0 {
		fmt.Print("ERROR {CalcPri}: The inputted transaction had size zero")
		return 1
	}
	pri := fee * factor / t.Sz()
	if pri == 0 {
...
func (tp *TxPool) Add(t *tx.Transaction) {
	...
	if t == nil {
		fmt.Printf("ERROR {tp.Add}: received a nil transaction.\n")
		return
	}
	if tp.Ct.Load() >= tp.Cap {
		fmt.Printf("ERROR {tp.Add}: capacity exceeded.\n")
		return
	}
	pri := CalcPri(t)
	tp.CurPri.Add(pri)
...
func (tp *TxPool) ChkTxs(remover []*tx.Transaction) {
	...
	for _, tx := range remover {
		if tx == nil {
		fmt.Printf("ERROR {ChkTxs} : received nil as input.\n")
		return
	}
	}
	rmvTxs := tp.TxQ.Rmv(remover)
	for _, tx := range rmvTxs {
		pri := CalcPri(tx)
		tp.CurPri.Sub(pri)
		tp.Ct.Dec()
	}
