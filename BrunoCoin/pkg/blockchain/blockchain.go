...
func (bc *Blockchain) Add(b *block.Block) {
	...
	defer bc.Unlock()
	if (b == nil) {
		fmt.Printf("ERROR {bc.Add}: received nil input")
		return
	}
	// find the previous node
	prevNode := bc.blocks[b.Hdr.PrvBlkHsh]
	var utxo = prevNode.utxo
	for _, t := range b.Transactions {
		// remove any used utxo
		for _, txin := range t.Inputs {
			key := txo.MkTXOLoc(txin.TransactionHash, txin.OutputIndex)
			delete(utxo, key)
		}
		// add utxo from the transactions' outputs
		for i, txout := range t.Outputs {
			key := txo.MkTXOLoc(t.Hash(), uint32(i))
			utxo[key] = txout
		}
	}
	// craft a new blockchain node
	...
	if newBlock.depth > bc.LastBlock.depth {
		bc.LastBlock = newBlock
	}
	fmt.Printf("DEBUG : A new block has been added to " + bc.Addr + "\n")
}
...
func (bc *Blockchain) GetUTXOForAmt(amt uint32, pubKey string) ([]*UTXOInfo, uint32, bool) {
	...
	defer bc.Unlock()
	if amt < 0 {
		fmt.Printf("ERROR {GetUTXOForAmt}: The required amount is below zero\n")
		return nil, 0, false
	}
	utxoInfo := make([]*UTXOInfo, 0)
	var currAmt uint32 = 0
	for k, v := range bc.LastBlock.utxo {
		...
		if v.LockingScript == pubKey && !v.Liminal {
			currAmt += v.Amount
			txHash, idx := txo.PrsTXOLoc(k)
			utxoInfo = append(utxoInfo,
				&UTXOInfo{
					TxHsh:  txHash,
					OutIdx: idx,
					UTXO:   v,
	...
	if currAmt < amt {
		fmt.Printf("ERROR {GetUTXOForAmt}: Not enough balance. Current balance: %d. Required amount: %d\n", currAmt, amt)
		return nil, 0, false
	}
	return utxoInfo, currAmt - amt, true
}
